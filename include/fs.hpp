...
class FileSystem
{
  ...
  struct File
  {
    ...
    )
    {
      ...
      std::filesystem::path new_path = std::filesystem::path(path);
      if(name.has_value())
      {
        new_path = std::filesystem::path(path).parent_path() / name.value();
  ...
  struct Dir
  {
    ...
    static std::optional<std::string> edit(const std::string& path, const std::optional<std::string>& name)
    {
      try
      {
        if (!std::filesystem::is_directory(path))
